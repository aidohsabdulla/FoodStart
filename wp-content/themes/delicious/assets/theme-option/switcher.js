	...
	var styleSwitcher = {
		...
		initialize : function() {
			...
			this.initialized = true;
			   ...
			   less.modifyVars({
				   		skinColor: DELICIOUS_OPTION.color,
				   		fontFamily: DELICIOUS_OPTION.font
				    });
			
			// Style Switcher CSS
			//$("head").append($('<link rel="stylesheet">').attr("href", "assets/js/theme-option/style-switcher/assets/css/font-awesome.css"));
			$("head").append($('<link rel="stylesheet">').attr("href", DELICIOUS_OPTION.url+"/assets/theme-option/switcher.css"));
			...
			//$("head").append($('<link rel="stylesheet/less">').attr("href", DELICIOUS_OPTION.url+"/assets/assets/options/skin.less"));
			...
			$this.events();
				...
				/*setTimeout(function() {
					if ($.cookie("header") != null) {
						...
						$('header.header-style .theme-pager li').removeClass('current');
						//alert($.cookie("main_index"))

						$('.header-items-slider').eq($.cookie("main_index")).find('.header-items').eq($.cookie("index")).addClass('select-active')
						$('header.header-style .theme-pager li').eq($.cookie("main_index")).addClass('current');
						...
						$('header.header-style .theme-pager li').eq(0).addClass('current');
		...
		build : function() {
		    ...
		    optionColor = $this.lookColorChange(colors);
		    if (optionColor) {
			colors.push(optionColor);
		    }
		    //===
		    ...
		    //------------
		    var DataHexFlag = true;
		    colorList.find('li a').each(function() {

			var $getDataHexColor = $(this).attr("data-color-hex");
			if ($getDataHexColor == DELICIOUS_OPTION.color) {
			    DataHexFlag = false;
			...
			$this.layoutStyle(DELICIOUS_OPTION.layout);
			$this.stickyHeader(DELICIOUS_OPTION.header, '');
		},
		lookColorChange: function($colors) {
		    var flag = true;
		    $.each($colors, function(i, value) {
			if ($colors[i].Hex == DELICIOUS_OPTION.color) {
			    flag = false;
			}
		    });
		    var oColor = null;
        ...
        $(".theme-translate").click(function () {
			...
			stickOnScroll = function() {
				if ($.cookie('sticky') == "intelligent") {
					...
					$('header.header').removeClass('normal');
					$('header.header').next().addClass('top-m');
					$('header.header').addClass('intelligent');
					var pos = $(window).scrollTop();

					if (pos > headerHeight) {
						if (pos > st) {						
							$('header.header').addClass('simple');
							$('header.header.simple').removeClass('down');
							$('header.header.simple').addClass('fixed up');

						} else {
							$('header.header.simple').removeClass('up');
							$('header.header.simple').addClass('fixed down');

						}
						st = pos;

					} else {
						$('header.header.simple').removeClass('fixed down up simple');
					}
					...
					$('header.header').next().addClass('top-m');
					$('header.header').addClass('simple fixed');
					$('header.header').removeClass('down up');
					$('header.header').removeClass('intelligent');
					$('#wrapper').css({
						paddingTop : 0
					});
				}
					...
					$('header.header.simple').removeClass('fixed down up simple');
					$('header.header').addClass('normal');
					$('header.header').next().removeClass('top-m');
					$('header.header').removeClass('intelligent');
					$('#wrapper').css({
						paddingTop : 0
					});
